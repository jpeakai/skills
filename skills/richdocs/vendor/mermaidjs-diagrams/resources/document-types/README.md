...
This is a different scope from the rest of the skill. `mermaidjs-diagrams.md` and the other
...
|---|---|---|
| One fence | `mermaidjs-diagrams.md`, `color_theming.md`, `contrast_tooling.md` | Every `classDef` with a `fill:` must set `color:`. |
...
5. **ASCII-only node labels, `<br/>` for line breaks.** Not `\n`, not Unicode
   box-drawing or emoji inside labels. See `mermaidjs-diagrams.md` → *Common Pitfalls*.
